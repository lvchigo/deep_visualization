...
sys.path.insert(0, project_root + 'lib')    #faster rcnn lib
import caffe, cv2, os
import argparse

keyfile = '/home/chigo/working/research/faster-rcnn/py-faster-rcnn-master/python-visualization/keyfile/'
model_def = keyfile + 'mutilabel/faster_rcnn_test.pt'
model_weights = keyfile + 'mutilabel/faster_rcnn.caffemodel'
model_means = keyfile + 'imagenet/ilsvrc_2012_mean.npy'
labels_file = keyfile + 'mutilabel/Dict_mutilabel.txt'
...
def demo_oneimage_mutilabel_all(net, im_file, output_path, im_name):
    ...
    feat = net.blobs['rpn/output'].data[0]
    output_file = os.path.join( output_path, 'rpn_output.png' )
    vis_square(feat, output_file)

    feat = net.blobs['rpn_cls_score'].data[0]
    output_file = os.path.join( output_path, 'rpn_cls_score.png' )
    vis_square(feat, output_file)

    feat = net.blobs['rpn_bbox_pred'].data[0]
    output_file = os.path.join( output_path, 'rpn_bbox_pred.png' )
    vis_square(feat, output_file)

    feat = net.blobs['rpn_cls_score_reshape'].data[0]
    output_file = os.path.join( output_path, 'rpn_cls_score_reshape.png' )
    vis_square(feat, output_file)

    feat = net.blobs['rpn_cls_prob'].data[0]
    output_file = os.path.join( output_path, 'rpn_cls_prob.png' )
    vis_square(feat, output_file)

    feat = net.blobs['rpn_cls_prob_reshape'].data[0]
    output_file = os.path.join( output_path, 'rpn_cls_prob_reshape.png' )
    vis_square(feat, output_file)

    featlist = net.blobs['rois'].data[0]
    plt.clf()
    #plt.figure(figsize=(15, 3))
    plt.plot(featlist.flat)
    output_file = os.path.join( output_path, 'rois.png' )     
    ...
    plt.subplot(4, 1, 4)
    _ = plt.hist(featlist.flat[featlist.flat > 0], bins=100)
    output_file = os.path.join( output_path, 'fc6_fc7.png' )     
    plt.savefig(output_file)

    featlist = net.blobs['cls_score'].data[0]
    plt.clf()
    plt.subplot(3, 1, 1)
    plt.plot(featlist.flat)

    featlist = net.blobs['bbox_pred'].data[0]
    plt.subplot(3, 1, 2)
    plt.plot(featlist.flat)

    featlist = net.blobs['cls_prob'].data[0]
    plt.subplot(3, 1, 3)
    plt.plot(featlist.flat)
...
def parse_args():
    ...
    parser.add_argument('--output', dest='output_path', default='/home/chigo/working/research/faster-rcnn/py-faster-rcnn-master/python-visualization/sv_faster_rcnn/')
